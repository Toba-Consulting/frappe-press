...
RUN --mount=type=cache,target=/var/cache/apt curl -fsSL 'https://mariadb.org/mariadb_release_signing_key.pgp' | gpg --dearmor -o /etc/apt/keyrings/mariadb.gpg  \
    && echo "deb [signed-by=/etc/apt/keyrings/mariadb.gpg] https://mirror.rackspace.com/mariadb/repo/10.11/debian focal main main/debug" | tee /etc/apt/sources.list.d/mariadb.list \
    && apt-get update \
    && apt-get install --yes --no-install-suggests --no-install-recommends \
    gdb \
    mariadb-server \
